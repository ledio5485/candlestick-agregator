...
- the last quotes price, that was received (closePrice) 
...
## Assumptions 
- When a quote is received and no instrument is stored, then it will be discarded. This may lead to some issues during the start, when the existing instruments are already active in the partner service, but a quote event is arrived before the related instrument.
- If there were no candlesticks in a pipeline before, the default candlestick with the negative price values will be returned.
- the persistence technique is just having 2 in-memory Map, and search by isin. a better approach would be using a time series DB
- the service can be split in 2 (micro)services to be horizontally scalable (event-listeners & candlestick-aggregator) this approach help us scaling (when needed) different services independently
